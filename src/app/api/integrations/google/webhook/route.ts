...
/**
 * Deletes events from database that no longer exist in Google Calendar
 */
async function deleteMissingEvents(
  ...
  try {
    ...
    console.log(`🗑️ Webhook: Found ${eventsToDelete.length} events to delete (no longer in Google Calendar)`);
    ...
    for (const eventToDelete of eventsToDelete) {
      if (eventToDelete.is_soradin_created && eventToDelete.appointment_id && allowExternalEdits) {
        console.log(`⚠️ Webhook: Soradin appointment ${eventToDelete.appointment_id} was deleted in Google Calendar - cancelling`);
        
        const { error: cancelError } = await supabaseAdmin
          .from("appointments")
          .update({
            status: "cancelled",
            notes: `Cancelled externally via Google Calendar on ${new Date().toISOString()}`,
          })
